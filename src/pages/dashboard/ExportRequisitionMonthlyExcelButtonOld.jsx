...
export default function ExportRequisitionMonthlyExcelButton({ groupId, searchValues }) {
  ...
  // 🔥 Bạn thiếu dòng này → REACT CRASH → mình thêm lại
  const [currency, setCurrency] = useState('VND');

  const createdDateFormatted = createdDate;

  // Fetch lần đầu để hiển thị trong UI
  useEffect(() => {
    if (!groupId) {
  ...
  // =================================================
  // 🔥 EXPORT FILE EXCEL — KHÔNG ĐỤNG LOGIC CỦA BẠN
  // =================================================
  const exportToExcel = async () => {
    try {
      ...
      // ============================================
      // ✔ TITLE SỬA THEO YÊU CẦU CỦA BẠN
      // ============================================
      const titleRow = new Array(totalCols).fill('');
      titleRow[0] = `REQUEST MONTHLY (${createdDateFormatted})`;
