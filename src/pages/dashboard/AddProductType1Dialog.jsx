...
export default function AddProductType1Dialog({
  open,
  onClose,
  onAdd, // async (name) => Promise
  disabled = false,
}) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // success toast để parent show là chính; dialog tự đóng cho gọn
      onClose?.();
    } catch (err) {
      console.error(err);
  ...
  const titleName = name?.trim() || 'Unknown';
          ...
          <Stack spacing={2}>
            <Box sx={{ ...subtleCardSx, p: 2 }}>
              ...
              <Typography sx={{ color: 'text.secondary', fontSize: 13, mt: 0.3 }}>
                Keep naming consistent for clean filtering
              </Typography>
              ...
              >
                <Stack direction="row" spacing={1} alignItems="flex-start">
                  ...
                  <Typography sx={{ fontSize: 12.5, color: 'text.secondary' }}>
                    <b>Tip:</b> Use short, standardized names (avoid extra symbols).
                  </Typography>
                </Stack>
              </Box>
            </Box>
      ...
      >
        ...
        <DialogContent sx={{ pt: 1 }}>
          <Typography sx={{ color: 'text.secondary', fontSize: 13.5 }}>
            Create Product Type 1 with name <b>{titleName}</b>?
          </Typography>
